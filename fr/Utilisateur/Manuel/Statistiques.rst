...
Au lancement du module, un tableau de bord propose sur une période d’un mois un 
ensemble de statistiques généralistes.

Les trois autres onglets proposent des statistiques qui concernent respectivement 
les connexions des utilisateurs, les téléchargements de documents et enfin l’activité 
du site.

Tableau de bord
---------------
Les statistiques proposées par le tableau de bord sont globales pour toutes les 
bases et collections publiées à partir du site et sont données sur un mois glissant.
...
**Voici une liste des tableaux disponibles et significations**
  ...
  * Cumul téléchargements et connexions: (Tableau)
    Nombre total de téléchargements, documents toutes sous résolutions sur un mois 
...
Les trois onglets suivants: `Connexion`_, `Téléchargements`_ et `Activité du Site`_, 
se présentent de la même façon: L'utilisateur doit sélectionner 3 critères afin 
...
Connexion
----------
L'utilisateur clique sur l'onglet "Connexion" s'il souhaite voir combien de connexions 
ont eu lieu sur son système Phraseanet. 
...
Utiliser un intervalle proposé par les trois calendriers. Il faut utiliser le premier 
calendrier de la liste pour remonter vers un passé plus lointain.
...
Bases 
*****
Deuxième étape: sélectionner les bases et collections sur lesquelles on veut des 
statistiques.

Cliquer sur "toutes les bases" pour afficher la liste des bases de données disponibles.
Deux boutons permettent de sélectionner/désélectionner toutes les bases.
...
Le menu contextuel (flèche dans le coin gauche en bas) permet d’afficher la liste 
des collections de la base, afin d’affiner les rapports, en décochant les collections 
à exclure.

.. note:: Les options sélectionnées pour les intervalles et bases/collections sont mémorisées 
...
Liste de toutes les connexions faites par chaque utilisateur présentée par défaut 
sur un critère chronologique
...
Téléchargements
---------------
L'onglet Téléchargement se présente de la même manière que l'onglet `Connexion`_: 
L'utilisateur doit d'abord sélectionner la Période, puis les Bases et collections.

Ensuite, le choix pour le type de rapport change. En effet, il est possible d'afficher 
les statistiques en se basant sur les rapports: 

  * Par utilisateurs:
  
Pour chaque utilisateur ayant réalisé des téléchargements, ce rapport affiche le 
nombre et poids des documents et sous résolutions.
...
Active une option permettant de rechercher si un document en particulier à partir 
de son Recordid ou si un document pour lequel un terme à chercher est présent 
dans un champs a été téléchargé sur la période.

*Exemple de rapport de statistiques par document sur la base:*

  .. image:: ../../images/Statistiques-Telechargements.jpg
	   :height: 250 px
	   :width: 480 px
	   :alt: alternate text
	   :align: center
...
Activité du Site
----------------
Ce dernier onglet se présente également de la même manière que les deux précédents. 
L'utilisateur doit aussi choisir la période et la base, ensuite les types de rapports 
...
Liste des questions les plus posées et nombre de réponses retournées par Phraseanet.
...
*Exemple de rapport incluant les données relatives aux questions les plus posées:*
...
La plupart des tableaux, listes et graphiques sont imprimables en cliquant sur le 
symbole de l'imprimante.   

Les rapports sont générés par base de données et tiennent compte des filtres 
d’exclusion de collections.

Pour naviguer dans la liste « onglets » des bases, positionner le curseur en 
début ou fin de liste.

  .. image:: ../../images/Statistiques-Bases.jpg
	   :height: 30 px
	   :width: 600 px
	   :alt: alternate text
	   :align: center
...
Les rapports, lorsqu’ils sont constitués d’un grand nombre de lignes, sont répartis 
sur plusieurs pages.
